...
//  marketAppIOS
//
//  Created by andrey.pogodin on 04.12.2023.
//
...
public struct PushData : Encodable {
    ...
    public var newbuttons: [String:newButton]?
}
public struct newButton : Encodable{
...
class library {
    public enum PushNotificationService {
        ...
        private static func getData(aps: [String: AnyObject], account: String) -> PushData {
            ...
            var buttons = [String:newButton]()
            if let apsButtons = apsButtons{
                for b in apsButtons{
                    let but = b as! [String:String]
                    let id = but["id"]
                    let button = newButton(id: id ?? "0", label: but["label"] ?? "", action: but["action"] ?? "", link: but["link"] ?? "")
                    buttons[id!] = button
                }
            }
    ...
    public enum SessionService {
        ...
        public static func getUrls(){
            let devClients: [String] = ["ilya_client", "maxim_t_client", "margarita_client", "zhan", "andrey_pogodin2"]
        ...
        public static func createSession(account: String) {
            
            let urlFromMap = "https://dev.ext.enkod.ru/sessions"
            print(urlFromMap)
        ...
        static func startSession(account: String, sessionID: String) {
            ...
            let urlFromMap = "https://dev.ext.enkod.ru/sessions/start"
            guard let url = URL(string: urlFromMap) else { return }
            var urlRequest = URLRequest(url: url)
            urlRequest.addValue(account, forHTTPHeaderField: "X-Account")
            ...
            URLSession.shared.dataTask(with: urlRequest) { (data, response, error) in
                if data != nil {
                    ...
                    if let token = PushNotificationService.getToken {
                        print("yes token in  startSession")
        ...
        public static func subscribePush(account: String, token: String, sessionID: String) {
            ...
            var urlFromMap = "https://dev.ext.enkod.ru/mobile/subscribe"
            guard let url = URL(string: urlFromMap) else { return }
            var urlRequest = URLRequest(url: url)
            urlRequest.httpMethod = "POST"
            urlRequest.addValue(account, forHTTPHeaderField: "X-Account")
    ...
    public struct Order {
     ...
     class Tracker {
        ...
        public func prepareRequest(_ method: String, _ url: String, _ body: Data?) -> URLRequest?{
            let account = library.PushNotificationService.getAccount
            let session = library.PushNotificationService.getSessionID
        ...
        private func subscribeNew(subscriberInfo: [String:Any]) throws {
            ...
            guard let urlRequest = prepareRequest("POST", "https://dev.ext.enkod.ru/subscribe", json) else { return }
            URLSession.shared.dataTask(with: urlRequest) {(data, response, error) in
                if data != nil {
                    DispatchQueue.main.async {
        ...
        private func updateCurrent(person: PersonData, email: String, phone: String){
            ...
            UserDefaults.standard.set(person.session, forKey: library.PushNotificationService.sessionID)
            library.SessionService.startSession(account: library.PushNotificationService.getAccount!, sessionID: person.session)
        ...
        fileprivate func updateContacts(_ email: String, _ phone: String){
            ...
            urlRequest.httpMethod = "PUT"
            urlRequest.addValue(library.PushNotificationService.getAccount!, forHTTPHeaderField: "X-Account")
            urlRequest.addValue(library.PushNotificationService.getSessionID!, forHTTPHeaderField: "X-Session-Id")
        ...
        fileprivate func subscribe(_ sub: Subscriber) throws {
            ...
            guard library.PushNotificationService.getSessionID != "" else { throw TrackerErr.emptySession }
            ...
            URLSession.shared.dataTask(with: urlRequest) { [self](data, response, error) in
                ...
                if response?.statusCode == 404{
                    ...
                    self.updateCurrent(person: PersonData(
                        ...
                        session:    library.PushNotificationService.getSessionID ?? ""),
                        email:      sub.email!,
                        phone:      sub.phone!)
                }
            }.resume()
        ...
        fileprivate func logOut(){
            ...
            library.PushNotificationService.unsubscribePush()
            library.PushNotificationService.dropSession()
            library.SessionService.createSession(account: library.PushNotificationService.getAccount!)
            cart = Cart()
            favourite = Favourite()
        }
